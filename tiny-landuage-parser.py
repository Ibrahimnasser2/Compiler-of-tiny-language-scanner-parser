    ...
    def CheckTiny(self):
        ...
        if self.children:
            for l in range(len(self.children) - 1):
                if 'if' in self.children[l].data['line'] or 'else' in self.children[l].data['line'] or 'repeat' in                         self.children[l].data['line']:
                    continue
    ...
    try:
        ...
        for n, o in enumerate(List):
            if List[n]['Token'] == 'read':
                ...
                while stack[-1]['Type'] == 'op' or stack[-1]['Type'] == 'assign' or stack[-1]['Type'] == 'read' or                         stack[-1]['Type'] == 'write':
                    stack.pop(-1)
                if Flag:
...
line_number_bar = Text(inputframe, width=4, padx=3, takefocus=0, border=0,
                       background='white', state='disabled', wrap='none', fg='black')
line_number_bar.pack(side='left', fill='y')
text = Text(inputframe, bg='white', fg='black',width=4)
...
OutputText = Text(OutputFrame, takefocus=0, relief=RAISED, border=0, background='Dim Grey', wrap='none', fg='white')
